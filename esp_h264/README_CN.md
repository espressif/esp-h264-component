...
| 分辨率        | 原始格式                  | 内存 (Byte) | fps (纯 C) | fps (汇编) |
|------------|-----------------------|-----------|-----------|----------|
...
当使用 ESP32-S31 时，esp_h264 提供两种预编译库，默认使用纯 C 实现的预编译库。启用 `CONFIG_ESP_H264_S31_USE_ASM` 配置项后，将切换至经过汇编优化的预编译库，可获得一定的性能提升，但会占用部分 CPU Core 1 的计算资源。因此，不建议在启用该选项时使用双任务解码。
...
### Q: 在启用 PSRAM 加密的 ESP32-P4 上，如何使用硬件编码器？

**A:** ESP-H264 硬件编码器使用 DMA 相关的视频缓冲区。开启 PSRAM 加密时，位于 PSRAM 的编码输入和输出缓冲区需要分配在非加密内存区域，否则硬件编码可能失败。

为支持加密场景，组件做了如下处理：

- 启用 `CONFIG_SPIRAM_ENC_EXEMPT` 后，`ESP_H264_MEM_SPIRAM` 包含 `MALLOC_CAP_SPIRAM_NO_ENC` capability，可从非加密 PSRAM 区域分配缓冲区。
- `esp_h264_aligned_malloc()` 和 `esp_h264_aligned_calloc()` 会按 cache 行对齐分配大小，并为缓冲区添加 `MALLOC_CAP_CACHE_ALIGNED` capability。

使用时请注意：

1. 建议始终通过 `esp_h264_aligned_malloc()` 或 `esp_h264_aligned_calloc()` 分配编码器缓冲区，并使用 `ESP_H264_MEM_SPIRAM` 作为 capability 参数，以保证内存区域与 cache 对齐要求正确。
2. 非加密区的大小由 `CONFIG_SPIRAM_ENC_EXEMPT_SIZE` 决定。缓冲区大小取决于处理的最大分辨率，应根据组件所需缓冲区总量配置。区域不足时，内存分配会失败；该值大于等于实际 PSRAM 容量时，非加密区会被禁用。
